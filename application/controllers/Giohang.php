...
class Giohang extends CI_Controller
{
    ...
    {
        $this->data['title'] = 'Anipat - Your cart';
    ...
    function check_mail()
    {
        ...
        if ($this->Mcustomer->customer_detail_email($email)) {
            $this->form_validation->set_message(__FUNCTION__, 'Email is already a member, Please login or enter another Email!');
            return FALSE;
        }
        return TRUE;
    }
    ...
    public function info_order()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->data['title'] = 'Anipat - Order Information';
    ...
    public function thankyou()
    {
        if ($this->session->userdata('info-customer') || $this->session->userdata('sessionKhachHang')) {
            ...
            $this->email->to($list);
            $this->email->subject(' Anipat Store');
        ...
        $this->data['title'] = 'Anipat Store.vn - Order results';
    ...
    public function coupon()
    {
        ...
        $html = '';
        if ($this->session->userdata('coupon_price')) {
            $html .= '<p>Only 1 discount code can be applied per order!!</p>';
        } else {
            if (empty($_POST['code'])) {
                $html .= '<p>Please enter Discount Code if available !!</p>';
            } else {
                ...
                if (empty($getcoupon)) {
                    // echo "123";
                    $html .= '<p>Discount code does not exist!</p>';
                }
                foreach ($getcoupon as $value) {
                    if ($value['code'] == $coupon) {
                        if (strtotime($value['expiration_date']) <= strtotime($today)) {
                            $html .= '<p>Discount code ' . $value['code'] . ' has expired from date ' . $value['expiration_date'] . ' !</p>';
                        } else if ($value['limit_number'] - $value['number_used'] == 0) {
                            $html .= '<p>Discount code' . $value['code'] . ' the number of entries has expired !</p>';
                        } else if ($value['payment_limit'] >= $money) {
                            $html .= '<p> This discount code only applies to orders from ' . number_format($value['payment_limit']) . ' đ upwards !</p>';
                        } else {
                            $html .= '<script>document.location.reload(true);</script>'; //// <p>Discount code'.$value['code'].' has been activated !</p>
                            // $html = "okeoke";
