...
class Coupon extends CI_Controller {
	...
	public function insert(){
		...
		$this->form_validation->set_rules('discount', 'discount amount', 'required');
		$this->form_validation->set_rules('limit_number', 'limit_number', 'required');
		$this->form_validation->set_rules('code', 'Discount code', 'required|is_unique[db_discount.code]|min_length[5]|max_length[10]');
		if ($this->form_validation->run() == TRUE) 
		{
			...
			$this->session->set_flashdata('success', 'Add discount code successfully');
			redirect('admin/coupon','refresh');
		} 
		else 
		{
			$this->data['view']='insert';
			$this->data['title']='Add New Discount Code
			';
        	$this->load->view('backend/layout', $this->data);
		}
	}
	...
	public function update($id){
		...
		$this->load->library('session');
		$this->form_validation->set_rules('discount', 'discount amount', 'required');
		$this->form_validation->set_rules('code', 'discount code', 'required|min_length[6]|max_length[10]');
		if ($this->form_validation->run() == TRUE) 
		{
			$mydata= array(
				'code' =>$_POST['code'],
			...
			$this->session->set_flashdata('success', 'Update discount code successfully');
			redirect('admin/coupon/','refresh');
		} 
		$this->data['view']='update';
		$this->data['title']='Update Discount Code
		';
		$this->load->view('backend/layout', $this->data);
	}
	...
	public function trash($id){
		...
		$this->Mcoupon->coupon_update($mydata, $id);
		$this->session->set_flashdata('success', 'Successfully deleted discount code in trash');
		redirect('admin/coupon','refresh');
	}
	...
	public function recyclebin(){
		...
		$this->data['title']='Trash discount code';
		$this->load->view('backend/layout', $this->data);
	}
	...
	public function restore($id){
		...
		$this->session->set_flashdata('success', 'discount code recovery successfully');
	...
	public function delete($id){
		...
		$this->session->set_flashdata('success', 'Successfully deleted discount code');
		redirect('admin/coupon/recyclebin','refresh');
	}
